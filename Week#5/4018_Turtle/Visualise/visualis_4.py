...
class TurtleAnimator:
    ...
    def reset_env(self):
        """Повертає всі змінні та графік у початковий стан"""
        self.dp = np.full((self.n, self.m), np.nan)
        self.calc_i, self.calc_j = 0, 0
        self.path = []
        self.current_path_step = 0
        self.phase = "CALCULATING"

        self.ax.clear()  # Повністю очищуємо графік
        self.text_elements = {}
        self.visual_patches = []  # Список для видалення підсвітки та стрілок

        self._setup_plot()
        self._draw_initial_grid()
        self.btn.label.set_text("Calculate Next Cell")
    ...
    def _setup_plot(self):
        ...
        self.ax.set_title("Крок 1: Заповнення таблиці DP", fontsize=16)
    ...
    def _draw_initial_grid(self):
        for i in range(self.n):
            for j in range(self.m):
                ...
                # Збільшені шрифти
                self.ax.text(j, i - 0.2, f'Acid: {self.acid[i, j]}',
                             ha='center', va='center', color='blue', fontsize=14)
                self.text_elements[(i, j)] = self.ax.text(j, i + 0.2, 'DP: ?',
                                                          ha='center', va='center',
    ...
    def calculate_step(self):
        ...
        self.text_elements[(i, j)].set_fontsize(16)

        patch = plt.Rectangle((j - 0.5, i - 0.5), 1, 1, color='yellow', alpha=0.2)
        self.ax.add_patch(patch)
        self.visual_patches.append(patch)
        ...
        if self.calc_i == self.n:
            ...
            self.ax.set_title("Таблицю заповнено! Починаємо шлях", color='green', fontsize=16)
    ...
    def move_step(self):
        if self.current_path_step < len(self.path) - 1:
            ...
            ann = self.ax.annotate('', xy=(x2, y2), xytext=(x1, y1),
                                   arrowprops=dict(arrowstyle='->', color='green', lw=4, mutation_scale=25))
            self.visual_patches.append(ann)
            self.current_path_step += 1

            if self.current_path_step == len(self.path) - 1:
                self.phase = "FINISHED"
                self.btn.label.set_text("Reset")
                self.ax.set_title(f"Фініш! Сума: {int(self.dp[self.n - 1, self.m - 1])}. Натисніть Reset.", fontsize=16)
    ...
    def process_click(self, event):
        if self.phase == "CALCULATING":
            ...
            self.ax.set_title("Крок 2: Відновлення шляху", fontsize=16)
...
if __name__ == "__main__":
    acid_data = np.array([[5, 9, 4, 3], [3, 1, 6, 9], [8, 6, 8, 12]])
